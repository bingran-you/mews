...
import * as identityModule from "../../src/mews/engine/daemon/identity.js";
import * as httpModule from "../../src/mews/engine/daemon/http.js";
import * as pollerModule from "../../src/mews/engine/daemon/poller.js";
import {
  parseDaemonArgs,
  runDaemon,
} from "../../src/mews/engine/daemon/runner-skeleton.js";
import { extractBackendFlag } from "../../src/mews/cli.js";

const tempRoots: string[] = [];
const ORIGINAL_MEWS_DIR = process.env.MEWS_DIR;
const ORIGINAL_MEWS_HOME = process.env.MEWS_HOME;
...
afterEach(() => {
  ...
  if (ORIGINAL_MEWS_DIR === undefined) delete process.env.MEWS_DIR;
  else process.env.MEWS_DIR = ORIGINAL_MEWS_DIR;
  if (ORIGINAL_MEWS_HOME === undefined) delete process.env.MEWS_HOME;
  else process.env.MEWS_HOME = ORIGINAL_MEWS_HOME;
...
function makeTempRoot(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `mews-daemon-${prefix}-`));
  tempRoots.push(dir);
  return dir;
}
...
describe("runDaemon end-to-end skeleton", () => {
  ...
  it("refreshes the service lock heartbeat and writes runtime status while running", async () => {
    const mewsDir = makeTempRoot("status");
    process.env.MEWS_DIR = mewsDir;
    delete process.env.MEWS_HOME;
    process.env.PATH = "";
    ...
    const runnerHome = join(mewsDir, "runner");
    const lockPath = join(
      runnerHome,
      "locks",
...
describe("cli dispatcher routes run / run-once / daemon to the TS runner", () => {
  ...
  it("routes `daemon` to runDaemon with once=false", async () => {
    ...
    vi.doMock("../../src/mews/engine/daemon/runner-skeleton.js", () => ({
      runDaemon: runDaemonSpy,
    }));
    const { runMews } = await import("../../src/mews/cli.js");
    await runMews(["daemon", "--poll-interval-secs", "30"], () => {});
  ...
  it("routes `run` to runDaemon with once=false and strips any stray --backend flag", async () => {
    ...
    const { runMews } = await import("../../src/mews/cli.js");
    await runMews(["run", "--backend=rust", "--verbose"], () => {});
