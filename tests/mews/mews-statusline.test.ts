/**
 * mews statusline: single-line summary read from `~/.mews/inbox.json`.
 *
 ...
 *   - runs the built `dist/mews-statusline.js` bundle with a fake
 *     `$MEWS_DIR` cache and measures wall-clock time.
 */
...
import { renderStatusline } from "../../src/mews/engine/statusline.js";

function mkMewsDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "mews-sl-"));
  return dir;
}
...
describe("renderStatusline", () => {
  it("returns a line with 0 need-you when counts are all zero", () => {
    ...
    expect(out.line).toBe("/mews: ⚠ 0 need-you · ");
  ...
  it("formats human-only summary with no bell on first run", () => {
    ...
    expect(out.line).toBe("/mews: ⚠ 2 need-you · ");
    expect(out.ring).toBe(false);
  });
  ...
  it("formats new-only summary broken down by type", () => {
    ...
    const out = renderStatusline(c, null);
    expect(out.line).toBe("/mews: ⚠ 0 need-you · 2 PRs · 1 issues");
  });
  ...
  it("orders new-type breakdown by count desc", () => {
    ...
    const out = renderStatusline(c, null);
    expect(out.line).toBe("/mews: ⚠ 0 need-you · 3 PRs · 1 issues · 1 discussions");
  });
  ...
  it("rings on new-count increase", () => {
    ...
    expect(out.line).toBe("/mews: ⚠ 0 need-you · 5 PRs (+2 new)");
  ...
  it("does not ring when the poll timestamp is unchanged", () => {
    ...
    const out = renderStatusline(c, prior);
    expect(out.ring).toBe(false);
    expect(out.line).toBe("/mews: ⚠ 0 need-you · 5 PRs");
  });
});

describe("mews-statusline dist bundle", () => {
  ...
  it("prints a summary line from a fake cache file", () => {
    if (!existsSync(BUNDLE_PATH)) {
      ...
      console.warn(
        "dist/mews-statusline.js missing — run `pnpm build` before this test",
      );
      return;
    }
    const dir = mkMewsDir();
    try {
      const inbox = {
        last_poll: "2026-04-16T20:00:00Z",
        notifications: [
          { id: "a", type: "PullRequest", mews_status: "new" },
          { id: "b", type: "PullRequest", mews_status: "new" },
          { id: "c", type: "Issue", mews_status: "new" },
          { id: "d", type: "PullRequest", mews_status: "human" },
        ],
      };
      ...
      const result = spawnSync(process.execPath, [BUNDLE_PATH], {
        env: { ...process.env, MEWS_DIR: dir },
        encoding: "utf-8",
        input: "",
      });
      expect(result.status).toBe(0);
      expect(result.stdout).toMatch(/\/mews:/u);
  ...
  it("cold-starts under 200ms (target <30ms; 200ms guards against regressions)", () => {
    ...
    try {
      ...
      console.log(
        `mews-statusline cold-start: median ${median}ms across ${runs} runs (timings: ${timings.join(", ")})`,
      );
