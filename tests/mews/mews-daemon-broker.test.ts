...
} from "../../src/mews/engine/daemon/broker.js";
...
function makeTempDir(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `mews-broker-${prefix}-`));
  tempRoots.push(dir);
  return dir;
}
...
describe("SHIM_SCRIPT", () => {
  it("references MEWS_BROKER_DIR and writes argv.txt", () => {
    expect(SHIM_SCRIPT).toContain("MEWS_BROKER_DIR");
