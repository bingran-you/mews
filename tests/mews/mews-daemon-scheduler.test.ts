...
function makeHome(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `mews-sched-${prefix}-`));
  tempRoots.push(dir);
  return dir;
}
...
describe("operator-repo routing helpers", () => {
  ...
  it("shouldRouteToOperatorRepo triggers only on self-maintenance", () => {
    const yes = [
      "@bingran-you please configure the mews-runner to",
      "can you fix the mews-runner service for me",
    ]
      .join("\n")
      .toLowerCase();
    ...
    const no =
      "please review this pull request — mews-runner already commented.".toLowerCase();
  ...
  it("readRoutingSnapshotText concatenates known files lowercased", () => {
    ...
    writeFileSync(join(dir, "subject.json"), "PLEASE RESTART MEWS-RUNNER");
    writeFileSync(join(dir, "latest-comment.json"), "Unrelated");
    const text = readRoutingSnapshotText(dir);
    expect(text).toContain("restart mews-runner");
  ...
  it("routeWorkspaceCandidate routes when a snapshot asks for reconfiguration", () => {
    ...
    writeFileSync(
      ...
      "@bingran-you please configure the mews-runner service",
    );
    const candidate = buildReviewRequestCandidate({
      repo: "o/r",
      number: 10,
