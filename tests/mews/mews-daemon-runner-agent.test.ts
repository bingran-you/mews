...
import {
  ...
  type AgentRequest,
  type AgentSpawner,
} from "../../src/mews/engine/daemon/runner.js";
...
function makeTempDir(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `mews-runner-${prefix}-`));
  tempRoots.push(dir);
  return dir;
}
...
function fakeRequest(overrides: Partial<AgentRequest> = {}): AgentRequest {
  const base: AgentRequest = {
    ...
    disclosureText: "Agent note: this is mews.",
  };
  return { ...base, ...overrides };
}

describe("parseResult", () => {
  it("parses status and summary from last MEWS_RESULT line", () => {
    const { status, summary } = parseResult(
      "working...\nMEWS_RESULT: status=handled summary=reviewed and replied",
    );
    expect(status).toBe("handled");
    expect(summary).toBe("reviewed and replied");
  });

  it("prefers the last MEWS_RESULT when multiple appear", () => {
    const { status } = parseResult(
      "MEWS_RESULT: status=failed summary=nope\nMEWS_RESULT: status=handled summary=ok",
    );
    expect(status).toBe("handled");
  });
  ...
  it("handles skipped status", () => {
    const { status } = parseResult(
      "MEWS_RESULT: status=skipped summary=already handled",
    );
...
describe("buildPrompt", () => {
  it("includes identity, task URL, local context, disclosure", () => {
    ...
    expect(prompt).toContain("Agent note: this is mews.");
    expect(prompt).toContain("MEWS_RESULT: status=");
  });
  ...
  it("keeps the prompt generic to mews rather than repo-specific workflows", () => {
    const prompt = buildPrompt(fakeRequest());
    expect(prompt).not.toContain("sync-proposal");
    expect(prompt).not.toContain("draft-node");
    expect(prompt).toContain('gh issue edit <number> --repo <owner>/<repo> --add-label "mews:<status>"');
  });
});
...
describe("buildAgentEnv", () => {
  it("prefixes shim dir to PATH and exports MEWS_* env vars", () => {
    ...
    expect(env.MEWS_BROKER_DIR).toBe("/opt/shim");
    expect(env.MEWS_SNAPSHOT_DIR).toBe("/snap");
    expect(env.MEWS_TASK_DIR).toBe("/task");
  });
});
...
describe("executeAgent", () => {
  it("writes prompt, invokes spawner, parses result", async () => {
    ...
    const spawner: AgentSpawner = async ({ outputPath }) => {
      ...
      writeFileSync(
        outputPath,
        "doing things\nMEWS_RESULT: status=handled summary=all good",
      );
      return { statusCode: 0 };
    };
