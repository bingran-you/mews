...
import {
  ...
  GhClient,
  GhExecError,
} from "../../src/mews/engine/runtime/gh.js";
...
describe("GhClient.removeLabel", () => {
  it("uses `gh issue edit --remove-label`", () => {
    const spawn = stubSuccess();
    const gh = new GhClient({ spawn });
    gh.removeLabel("owner/repo", 42, "mews:wip");
    expect(spawn).toHaveBeenCalledWith(
      "gh",
      [
        "issue",
  ...
  it("swallows failure (non-existent label etc.)", () => {
    ...
    const gh = new GhClient({ spawn });
    expect(() => gh.removeLabel("owner/repo", 42, "mews:wip")).not.toThrow();
  });
});
...
describe("GhClient.createLabel", () => {
  it("forwards color and description with --force", () => {
    const spawn = stubSuccess();
    const gh = new GhClient({ spawn });
    gh.createLabel("owner/repo", "mews:new", "0075ca", "Mews: new notification");
    expect(spawn).toHaveBeenCalledWith(
      "gh",
      [
        "label",
