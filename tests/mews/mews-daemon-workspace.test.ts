...
function makeTempDir(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `mews-ws-${prefix}-`));
  tempRoots.push(dir);
  return dir;
}
...
describe("WorkspaceManager.prepare", () => {
    ...
    expect(
      calls.some(
        (c) =>
          ...
          c.some((a) => a.includes("bob via mews-runner")),
      ),
    ).toBe(true);
    expect(
      calls.some(
  ...
  it("fetches PR head when prNumber is set", async () => {
    ...
    // Should fetch refs/pull/267/head:...mews-runner-pr-267.
    ...
    expect(addCall).toContain(
      "refs/remotes/origin/mews-runner-pr-267",
    );
  });
