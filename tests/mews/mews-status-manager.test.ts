...
import { runStatusManager } from "../../src/mews/engine/commands/status-manager.js";
import { resolveMewsPaths } from "../../src/mews/engine/runtime/paths.js";
import { GhClient } from "../../src/mews/engine/runtime/gh.js";
import { readActivityLog } from "../../src/mews/engine/runtime/activity-log.js";

const FIXTURE = join(__dirname, "..", "fixtures", "mews", "inbox-sample.json");

function mkMewsDir(): { dir: string; inbox: string; activity: string; claims: string } {
  const dir = mkdtempSync(join(tmpdir(), "mews-sm-"));
...
describe("status-manager: get", () => {
  let ctx: ReturnType<typeof mkMewsDir>;
  beforeEach(() => {
    ctx = mkMewsDir();
  });
  afterEach(() => rmSync(ctx.dir, { recursive: true, force: true }));

  it("returns mews_status for an existing id", async () => {
    const { stdout, io } = captureIO();
    const paths = resolveMewsPaths({ env: () => ctx.dir });
...
describe("status-manager: set", () => {
  ...
  it("removes all mews labels then adds the target label and patches inbox", async () => {
    const calls: string[][] = [];
...
describe("status-manager: ensure-labels", () => {
  ...
  it("creates all four mews labels with correct colors", async () => {
    const calls: string[][] = [];
    const gh = new GhClient({
      spawn: vi.fn().mockImplementation((_cmd, argv: string[]) => {
        calls.push([...argv]);
    ...
    expect(code).toBe(0);
    expect(stdout.join("\n")).toContain("Creating mews labels on owner/repo");
    ...
    expect(colorFor("mews:new")).toBe("0075ca");
    expect(colorFor("mews:wip")).toBe("e4e669");
    expect(colorFor("mews:human")).toBe("d93f0b");
    expect(colorFor("mews:done")).toBe("0e8a16");
  });
});
...
describe("status-manager: help", () => {
  it("prints usage and exits 0", async () => {
    ...
    expect(code).toBe(0);
    expect(stdout[0]).toMatch(/Usage: mews-status-manager/u);
  });
});
