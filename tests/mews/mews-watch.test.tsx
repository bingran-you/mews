/**
 * mews watch TUI tests — rendered via `ink-testing-library`.
 *
 * We exercise the pure view component `MewsWatch` with canned inbox +
...
import { MewsWatch } from "../../src/mews/engine/commands/watch.js";
import type {
  ActivityEvent,
  Inbox,
} from "../../src/mews/engine/runtime/types.js";
...
describe("MewsWatch view", () => {
  it("renders header with status counts", () => {
    ...
    const { lastFrame } = render(
      <MewsWatch inbox={inbox} events={[]} />,
    );
    const frame = strip(lastFrame());
    expect(frame).toMatch(/mews/u);
  ...
  it("renders live-feed transition events with from/to labels", () => {
    ...
    const { lastFrame } = render(
      <MewsWatch inbox={inbox} events={events} />,
    );
    const frame = strip(lastFrame());
  ...
  it("renders live-feed new events", () => {
    ...
    const { lastFrame } = render(
      <MewsWatch inbox={null} events={events} />,
    );
    const frame = strip(lastFrame());
    expect(frame).toMatch(/▸ NEW/u);
