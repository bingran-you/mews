...
import {
  ...
  mewsDaemonConfigSearchPaths,
  loadMewsDaemonConfig,
} from "../../src/mews/engine/runtime/config.js";

describe("loadMewsDaemonConfig", () => {
  let tmp: string;
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), "mews-daemon-cfg-"));
  });
...
`;
  ...
  it("searches ~/.mews/config.yaml", () => {
    const paths = mewsDaemonConfigSearchPaths(tmp);
  ...
  it("rejects malformed yaml with a descriptive error", () => {
    ...
    ).toThrow(/failed to parse mews daemon config/u);
  });
  ...
  it("env vars override yaml for concurrency knobs", () => {
    ...
    const envBag: Record<string, string> = {
      MEWS_MAX_PARALLEL: "30",
      MEWS_SEARCH_LIMIT: "25",
    };
    const cfg = loadMewsDaemonConfig({
      env: (name) => envBag[name],
      configPath,
  ...
  it("accepts the first existing yaml in the search order", () => {
    const primaryDir = join(tmp, ".mews");
    writeFileSync(
