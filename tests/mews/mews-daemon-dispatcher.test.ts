...
import { createBus, type BusEvent } from "../../src/mews/engine/daemon/bus.js";
import {
  Dispatcher,
...
import { tryClaim } from "../../src/mews/engine/daemon/claim.js";
...
function makeTempDir(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `mews-dispatch-${prefix}-`));
  tempRoots.push(dir);
  return dir;
}
...
describe("Dispatcher claim handling", () => {
  ...
  it("releases the claim after task completes", async () => {
    const spawner: AgentSpawner = async ({ outputPath }) => {
      writeFileSync(outputPath, "MEWS_RESULT: status=handled summary=ok");
      return { statusCode: 0 };
    };
    const env = setupDispatcher({ spawner });
    env.dispatcher.submit(fakeCandidate());
...
describe("Dispatcher execution", () => {
  ...
  it("marks completed and publishes task event", async () => {
    const spawner: AgentSpawner = async ({ outputPath }) => {
      writeFileSync(
        outputPath,
        "MEWS_RESULT: status=handled summary=all done",
      );
      return { statusCode: 0 };
    };
    const env = setupDispatcher({ spawner });
  ...
  it("falls through to the next runner on non-timeout failure", async () => {
    ...
    const spawner: AgentSpawner = async ({ spec, outputPath, stdoutPath }) => {
      ...
      writeFileSync(
        stdoutPath,
        "MEWS_RESULT: status=handled summary=secondary",
      );
