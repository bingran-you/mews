 ...
 * commands. We run them against a temporary `$MEWS_HOME` and assert
...
import { runCleanup } from "../../src/mews/engine/commands/cleanup.js";
import { runDoctor } from "../../src/mews/engine/commands/doctor.js";
import {
  resolveSelfStartCommand,
  runInstall,
} from "../../src/mews/engine/commands/install.js";
import { runStatus } from "../../src/mews/engine/commands/status.js";
import { ThreadStore } from "../../src/mews/engine/daemon/thread-store.js";
...
function makeHome(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `mews-cmd-${prefix}-`));
  tempRoots.push(dir);
  return dir;
}
...
describe("runDoctor", () => {
  it("prints the one-screen diagnostic regardless of identity", async () => {
    ...
    expect(out).toContain("mews-runner doctor");
...
describe("runStatus", () => {
  it("reports runtime/status.env keys when present", async () => {
    ...
    expect(code).toBe(0);
    const out = lines.join("\n");
    expect(out).toContain("mews-runner status");
...
describe("runInstall", () => {
  it("refuses to install-start without an explicit repo scope", () => {
    ...
    const code = runInstall([], {
      mewsDir: makeHome("install-missing-scope"),
      write: (line) => lines.push(line),
      checkCommand: () => true,
      checkGhAuth: () => true,
      spawn,
  ...
  it("re-invokes the current CLI for `mews start` instead of shelling to PATH", () => {
    const lines: string[] = [];
    const spawn = vi.fn(() => ({ status: 0 })) as unknown as typeof import("node:child_process").spawnSync;
    const code = runInstall(["--allow-repo", "owner/repo"], {
      mewsDir: makeHome("install"),
      write: (line) => lines.push(line),
      checkCommand: () => true,
      checkGhAuth: () => true,
      spawn,
...
describe("cli routing", () => {
  it("maps doctor/status/cleanup/start/stop/poll-inbox to TS specifiers", async () => {
    ...
    const cli = await import("../../src/mews/cli.js");
    expect(cli.MEWS_USAGE).toContain("doctor");
    expect(cli.MEWS_USAGE).toContain("poll-inbox");
