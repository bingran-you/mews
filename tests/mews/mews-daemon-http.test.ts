/**
 * Contract tests for the Phase 3b TypeScript port of the mews daemon
 * HTTP + SSE server.
 *
...
import {
  ...
  type SseBus,
  type SseEvent,
} from "../../src/mews/engine/daemon/sse.js";
...
function makeCtx(): Ctx {
  const dir = mkdtempSync(join(tmpdir(), "mews-http-"));
  return {
    dir,
...
describe("startHttpServer — route contracts", () => {
  ...
  it("GET / returns the dashboard HTML with the contract headers", async () => {
    ...
    expect(res.body.includes("<title>mews</title>")).toBe(true);
