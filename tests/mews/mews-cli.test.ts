...
describe("mews cli USAGE", () => {
  it("advertises every dispatched subcommand", () => {
    ...
    for (const sub of subcommands) {
      expect(MEWS_USAGE).toContain(sub);
    }
    expect(MEWS_USAGE).toContain("MEWS_DIR");
    expect(MEWS_USAGE).toContain("MEWS_HOME");
  });
});

describe("runMews dispatcher", () => {
  beforeEach(() => {
    vi.resetModules();
  });
  ...
  it("prints USAGE with no args", async () => {
    ...
    const code = await runMews([], output.write);
    expect(code).toBe(0);
    expect(output.lines).toEqual([MEWS_USAGE]);
  });
  ...
  it("errors with hint on unknown subcommand", async () => {
    const output = captureOutput();
    const code = await runMews(["no-such-thing"], output.write);
    expect(code).toBe(1);
    expect(output.lines[0]).toBe("Unknown mews command: no-such-thing");
    expect(output.lines[1]).toBe(MEWS_USAGE);
  });
  ...
  it("routes run / run-once / daemon through the TS daemon", async () => {
    ...
    const { runMews: freshRun } = await import(
      "../../src/mews/cli.js"
    );
  ...
  it("prints inline help for daemon-like subcommands without executing them", async () => {
    ...
    vi.doMock("../../src/mews/engine/commands/start.js", () => ({
  ...
  it("routes statusline through node + the separate dist bundle", async () => {
    ...
    vi.doMock("../../src/mews/engine/bridge.js", () => ({
      resolveMewsRunner: vi.fn(),
      resolveBundledMewsScript: vi.fn(() => {
  ...
  it("routes install through the TS install command", async () => {
    ...
    vi.doMock(
      "../../src/mews/engine/commands/install.js",
  ...
  it("routes poll through the TS port (not the runner)", async () => {
    ...
    vi.doMock("../../src/mews/engine/commands/poll.js", () => ({
      runPoll,
    }));
  ...
  it("routes watch through the TS port (not the bridge)", async () => {
    ...
    vi.doMock("../../src/mews/engine/commands/watch.js", () => ({
      runWatch,
    }));
  ...
  it("routes status-manager through the TS port (not the bash bridge)", async () => {
    ...
    vi.doMock("../../src/mews/engine/commands/status-manager.js", () => ({
      runStatusManager,
    }));
