/**
 * Tests for `src/products/mews/engine/runtime/store.ts`.
 *
 * Covers:
...
import {
  ...
  updateInbox,
  writeInbox,
} from "../../src/mews/engine/runtime/store.js";
import type { Inbox } from "../../src/mews/engine/runtime/types.js";

const FIXTURE = join(__dirname, "..", "fixtures", "mews", "inbox-sample.json");

function mkTmp(): string {
  return mkdtempSync(join(tmpdir(), "mews-store-"));
}
...
describe("readInbox / writeInbox", () => {
  ...
  it("emits every field in Rust-compatible key order", () => {
    ...
    const keyOrder =
      Array.from(
        firstEntryMatch![1].matchAll(/"(id|type|reason|repo|title|url|last_actor|updated_at|unread|priority|number|html_url|gh_state|labels|mews_status)"/gu),
      ).map((m) => m[1]);
      ...
      (current) => {
        ...
        return {
          ...
          notifications: current.notifications.map((n) =>
            n.id === "23576674030" ? { ...n, mews_status: "wip" as const } : n,
          ),
        };
      },
      { inboxPath },
